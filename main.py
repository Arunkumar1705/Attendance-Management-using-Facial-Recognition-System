...
window = tk.Tk()
window.title("Attendance Management using Facial Recognition System")
...
def subjectchoose():
    def Fillattendances():
        ...
        if time.time() < future:
            if sub == '':
                ...
                recognizer = recognizer = cv2.face.LBPHFaceRecognizer_create()  # cv2.createLBPHFaceRecognizer()
                try:
    ...
    windo = tk.Tk()
    windo.iconbitmap('peclogo.ico')
    windo.title("Enter subject name...")
    windo.geometry('700x400')
...
def admin_panel():
    ...
    def log_in():
        ...
        if username == 'panimalar' :
            if password == 'panimalar':
                win.destroy()
                import csv
                import tkinter
                root = tkinter.Tk()
                ...
                cs = "C:\\Users\\Arunk\\Desktop\\StudentDetails\\StudentDetails.csv"
...
message = tk.Label(window, text="Facial Recognition Attendance Managenment", bg="cyan", fg="black", width=46,
...
AP = tk.Button(window, text="Check Registered students",command=admin_panel,fg="black"  ,bg="cyan"  ,width=19 ,height=3, activebackground = "Red" ,font=('times', 15, ' bold '))
AP.place(x=1065, y=550)
